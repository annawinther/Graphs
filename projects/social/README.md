...
The formula to determine the first N pair in the friendship list is: `num_users * avg_friendships / 2.` Therefore, if we create 100 users with an average of 10 friends each, that'll be `(100 * 10) / 2 = 500`. So we would have to do 500 calls to `add_friendship()`.
